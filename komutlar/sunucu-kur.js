...
if(message.author.id !== message.guild.owner.id) return message.channel.send(' Bu komutu sadece **sunucunun sahibi** kullana bilir.')
...
await message.guild.channels.create("「📕」Kuralar", {type: "text", parent: message.guild.channels.cache.find(a => a.name === 'Önemli Kanallar').id,})
...
await message.guild.channels.create("「📷」Galari", {type: "text", parent: message.guild.channels.cache.find(a => a.name === 'Genel').id})
...
await message.guild.channels.create('Yetkili Kanalları', { type: "category" }).then(a => {
a.createOverwrite(message.guild.roles.cache.find(a => a.name === "@everyone"), {
 SEND_MESSAGES: false,
 VIEW_CHANNEL: false,
...
await message.guild.channels.create("「💬」Yetkili duyuru", {type: "text", parent: message.guild.channels.cache.find(a => a.name === 'Yetkiliye Özel').id})
await message.guild.channels.create("「🔒」Yetkili chat", {type: "text", parent: message.guild.channels.cache.find(a => a.name === 'Yetkiliye Özel').id})
  await message.guild.channels.create("「🔒」Yetkili komut", {type: "text", parent: message.guild.channels.cache.find(a => a.name === 'Yetkiliye Özel').id}) 
  await message.guild.channels.create("「🎤」Yetkili Özel", {type: "voice", parent: message.guild.channels.cache.find(a => a.name === 'Yetkiliye Özel').id})
...
enabled: true, 
guildOnly: false,
aliases: ['sunucukur','SUNUCU-KUR','SUNUCUKUR','sUnucu-KUR','Sunucu-Kur','SunucuKur'], 
permLevel: 0 
};
